...
const CSF_DOMAIN = 'apreal-volgograd.ru';
const CSF_RECIPIENT = 'vlg-ap@bk.ru';
const CSF_SENDER = 'wordpress@apreal-volgograd.ru';
...
function csf_block_legacy_cf7($spam)
{
    if ($spam || !class_exists('WPCF7_ContactForm')) {
        return $spam;
    }
    $form = WPCF7_ContactForm::get_current();
    if (!$form || !in_array((int) $form->id(), array(3261, 3317, 3497), true)) {
        return $spam;
    }
    return true;
}
...
{
    echo '<style>[id^="wpcf7-f3261-"],[id^="wpcf7-f3317-"],[id^="wpcf7-f3497-"]{display:none!important}</style>';
}
