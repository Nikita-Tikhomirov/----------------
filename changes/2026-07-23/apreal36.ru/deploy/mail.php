...
const APREAL_FORM_SENDER = 'wordpress@apreal36.ru';
...
$headers = [
    ...
    'From: apreal36.ru <' . APREAL_FORM_SENDER . '>',
...
if ($form_id === 'callback') {
    ...
    if ($comment !== '') {
        $message .= "<p><strong>Вопрос:</strong> {$comment}</p>";
    }
    $message .= "<p><strong>Страница:</strong> {$page}</p>";
} else {
    respond(false, 'Неизвестная форма.');
}

add_action('phpmailer_init', 'apreal_form_set_envelope_sender', 999);
$sent = wp_mail('info@apreal36.ru', $subject, $message, $headers);
